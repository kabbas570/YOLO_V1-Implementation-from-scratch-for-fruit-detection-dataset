...
def yolo_head(feats):
    ...
    box_xy = (feats[..., :2] + conv_index) / conv_dims * image_w
    box_wh = feats[..., 2:4] * image_w
...
def yolo_loss(y_true, y_pred):
    ...
    box_loss = 5 * box_mask * response_mask * K.square((label_xy - predict_xy) / image_w)
    box_loss += 5 * box_mask * response_mask * K.square((K.sqrt(label_wh) - K.sqrt(predict_wh)) / image_w)
